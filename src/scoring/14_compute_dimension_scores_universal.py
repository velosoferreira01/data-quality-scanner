...
def main():
    ...
    if args.run_id:
        ...
        row = con.execute(
            f"SELECT run_id FROM {stg}.dq_table_scores_u ORDER BY scanned_at DESC LIMIT 1"
        ).fetchone()
        if not row:
    ...
    con.execute(f"DELETE FROM {stg}.dq_table_dimension_scores_u WHERE run_id = ?", [run_id])
    ...
    con.execute(f"""
        ...
        SELECT
            ...
            LEAST(10.0, GREATEST(0.0,
                0.20 * COALESCE(t.score_completeness, 0.0) +
                0.15 * COALESCE(t.score_uniqueness, 0.0) +
                0.20 * COALESCE(c.avg_col_score_final, 0.0) +
                0.15 * COALESCE(c.avg_col_score_base, 0.0) +
                0.15 * LEAST(10.0, GREATEST(0.0, 10.0 + COALESCE(c.avg_col_score_rules, 0.0))) +
                0.15 * 0.0
            )) AS score_final
        FROM t
        LEFT JOIN c
